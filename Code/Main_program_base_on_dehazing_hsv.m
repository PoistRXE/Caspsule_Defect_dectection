...
%     temp=imresize(temp,BoundingBox);
%     imwrite(temp,['elements\lb_',num2str(z_p,'%2d'),'_',num2str(i,'%2d'),'.png']);
...
[m,n,p]=size(centriod_r_r);
area=zeros(b_t,1);
% area_all=zeros(b_t,1);
for i=1:b_t
    subplot(p*2,m,i),imshow(I_cell(i).cell);
    Area_temp=0;
    Area_Back_temp=0;
    ...
    BW=background_Area(Src_cell(i).cell);
    subplot(p*2,m,i+p*m),imshow(BW);
    for ii=1:mm
       for jj=1:nn
           if BW(ii,jj)
               Area_Back_temp=Area_Back_temp+1;
           end
       end
    end
%     area_all(i,1)=area(i,1);
    area(i,1)=area(i,1)/(r*c-Area_Back_temp);
end

defect_flag=zeros(p,m);
for t=1:p
    for i=1:m
        if(area(i+(t-1)*m,1)<0.45)
           defect_flag(t,i)=1;
        end
    end
end

% similarity_area=zeros(m,n,p);
% 
% for t=1:p
%    for i=1:m
%        for j=i:m
%            similarity_area(i,j,t)=abs(area_all(i+(t-1)*m,1)-area_all(j+(t-1)*m,1));
%        end
%    end
%    similarity_area(:,:,t)=similarity_area(:,:,t)+similarity_area(:,:,t)';
% end
% 
% similarity_area_flag=zeros(m,n,p);
% for t=1:p
%     for i=1:m
%        for j=1:n
%            if (similarity_area(i,j,t)>500)
%               similarity_area_flag(i,j,t)=1;
%            end
%        end
%     end
% end
